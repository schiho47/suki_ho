  ...
  return (
    <div className={`card ${styles.card}`} style={size}>
      <div className="row g-0">
        {img && (
          <div className={`col-md-6 img-fluid rounded-start ${styles.imageWrapper}`}>
        ...
        <div className={`${img ? 'col-md-6' : 'col-md-12'}`}>
          <div className={`card-body ${!img ? styles.bodyWithFooter : ''}`}>
            <div
              className={`d-flex justify-content-between align-items-center ${styles.headerRow}`}
            >
              <h3 className={`card-title mb-0 ${styles.cardTitle}`}>{title}</h3>
              {date && (
            ...
            {description && (
              <div
                className={`card-text ${styles.cardText} ${img ? styles.cardTextWithImage : ''}`}
              >
                {description}
              </div>
            )}
        ...
        <div className={linkBlockClassName}>
          {secondaryLink && secondaryLinkDescription && (
            <Link href={secondaryLink} target={secondaryLinkTarget}>
              <span>{`${secondaryLinkDescription} »`}</span>
            </Link>
          )}
          {secondaryLink && secondaryLinkDescription && (
